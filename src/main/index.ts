...
function deliverOpenPath(target: string): void {
  ...
  const send = (): void => win.webContents.send('app:open-path', { path: target, isDir })
...
function openEditorWindow(path: string): void {
  ...
  ed.webContents.on('did-finish-load', () => ed.webContents.send('editor:open-file', path))
  ed.on('closed', () => {
    editorWindow = null
  })
...
function createWindow(): BrowserWindow {
  ...
  ipcMain.handle('window:zoom', (_e, delta: number) => {
    ...
    if (delta === 0) {
      ...
      const next = Math.min(ZOOM_MAX, Math.max(ZOOM_MIN, wc.getZoomLevel() + delta))
