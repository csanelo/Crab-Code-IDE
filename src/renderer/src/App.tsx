...
function Workspace(): JSX.Element {
  const { state, createConversation, openProjectFromFolder, selectProject, setView } = useApp()
  ...
  const [leftWidth, setLeftWidth] = usePersistentState('leftWidth', LEFT_DEFAULT)
  const [rightWidth, setRightWidth] = usePersistentState('rightWidth', RIGHT_DEFAULT)
  const [terminalHeight, setTerminalHeight] = usePersistentState('terminalHeight', TERM_DEFAULT)
  const [resizing, setResizing] = useState<'left' | 'right' | 'terminal' | null>(null)
  ...
  useEffect(() => {
    return window.api.app.onOpenPath(({ path, isDir }) => {
      ...
      if (isDir) {
        const existing = state.repositories.find((r) => r.path && norm(r.path) === norm(path))
        ...
        const existing = state.repositories.find((r) => r.path && norm(r.path) === norm(dir))
        if (existing) selectProject(existing.id)
        else {
  ...
  }, [createConversation, openProjectFromFolder, setView, state.activeRepositoryId, activeRepo])
  ...
  useEffect(() => {
    return window.api.browser.onCommand((cmd) => {
      if (cmd.kind === 'navigate' && cmd.url) {
        ...
        emit('browser:capture', { kind: cmd.captureKind, requestId: cmd.requestId })
      }
    })
  }, [browserOpen])
  ...
  useEffect(() => {
    return onAppEvent('editor:fileCount', ({ count }) => setEditorFileCount(count))
  }, [])

  const startResizeLeft = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    e.preventDefault()
    setResizing('left')
  ...
  const startResizeRight = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    e.preventDefault()
    setResizing('right')
    const main = mainRef.current
    if (!main) return
    ...
    function onMove(ev: PointerEvent): void {
      const next = Math.min(RIGHT_MAX, Math.max(RIGHT_MIN, right - ev.clientX))
      setRightWidth(next)
    }
    function onUp(): void {
      setResizing(null)
  ...
  const startResizeTerminal = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    e.preventDefault()
    ...
    <div className="app">
      ...
      >
        <div className="app__row">
          <div className={`app__panel app__panel--left${leftOpen ? '' : ' app__panel--collapsed'}`}>
          ...
          <div className="app__rightside">
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    {browserOpen && (
                      <div className={`app__browser${browserFull ? ' app__browser--full' : ''}`}>
            ...
            {terminalMounted && (
              <>
                ...
                <div className={`app__terminal${terminalOpen ? ' app__terminal--open' : ''}`}>
                  <TerminalPanel
                    onClose={() => {
