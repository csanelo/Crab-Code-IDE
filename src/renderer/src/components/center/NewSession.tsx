...
import { getAccessLevel, setAccessLevel, type AccessLevel } from '../../lib/agentAccess'
import { getEditMode, setEditMode, type EditMode } from '../../lib/agentEditMode'
...
const ACCESS_LEVELS: { id: AccessLevel; key: TKey; descKey: TKey; icon: typeof ShieldCheck }[] = [
  { id: 'normal', key: 'access.normal', descKey: 'access.normalDesc', icon: ShieldCheck },
  { id: 'high', key: 'access.high', descKey: 'access.highDesc', icon: Zap }
]
...
function expandBuiltinCommand(cmd: string, rest: string): string | null {
  switch (cmd) {
    ...
    case 'code-review': {
      ...
      const focus = rest.replace(/^ultra\b/i, '').replace(/--fix\b/, '').trim()
      if (ultra) {
        return (
    ...
    case 'review':
      return (
        ...
        (rest ? ` focused on: ${rest}` : ' of the uncommitted changes (git diff against HEAD)') +
        '.\n' +
...
export function NewSession({
  ...
  const t = useT()
  const activeRepo = state.repositories.find((r) => r.id === state.activeRepositoryId) ?? null
  ...
  const [attachments, setAttachments] = useState<import('../../domain/types').Attachment[]>([])
  ...
  const AT_SOURCES: AtSource[] = [
    ...
    { id: 'changes', label: '@changes', hint: t('chat.at.changes'), icon: Diff },
    { id: 'git', label: '@git', hint: t('chat.at.git'), icon: GitBranch },
    { id: 'terminal', label: '@terminal', hint: t('chat.at.terminal'), icon: Terminal },
    { id: 'web', label: '@web', hint: t('chat.at.web'), icon: Globe },
    { id: 'codebase', label: '@codebase', hint: t('chat.at.codebase'), icon: Boxes }
  ]
  ...
  function applyMention(ref: FileRef): void {
    ...
    setMentions((prev) => (prev.some((p) => p.path === ref.path && p.name === ref.name) ? prev : [...prev, ref]))
    setMentionOpen(false)
  ...
  function addMentionByPath(path: string, name: string, isDir = false): void {
    ...
    setValue((v) => {
      const base = v === '' || v.endsWith(' ') || v.endsWith('\n') ? v : `${v} `
      return `${base}${name} `
    })
  }
  ...
  function renderHighlight(text: string): React.ReactNode {
    ...
    if (cmdMatch) {
      ...
      const known =
        SLASH_COMMANDS.some((c) => c.name === cmdName) || skills.some((s) => s.name === cmdName)
      if (known) {
        head.push(
    ...
    const allTokens = [...names, ...atLabels].sort((a, b) => b.length - a.length)
    if (allTokens.length === 0) return [...head, working + '\n']
    const escaped = allTokens.map((n) => n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
  ...
  const builtinMatches =
    slashQuery !== null
      ? SLASH_COMMANDS.filter((c) => c.name.startsWith(slashQuery.toLowerCase())).map((c) => ({
          name: c.name,
          title: t(c.titleKey),
          icon: c.icon,
          skill: false
  ...
  const skillMatches =
    slashQuery !== null
      ? skills
          ...
          .map((s) => ({ name: s.name, title: s.description, icon: Sparkles, skill: true }))
      : []
  const slashMatches = [...skillMatches, ...builtinMatches]
  const showSlash = slashOpen && slashMatches.length > 0

  useEffect(() => {
    void providersService.get().then((s) => {
      ...
      setModels(opts)
      const current =
        opts.find((o) => o.providerId === s.activeId && o.modelId === s.activeModel) ??
  ...
  async function pickModel(opt: ModelOption): Promise<void> {
    setActiveModel(opt)
    await providersService.setActive({ id: opt.providerId, model: opt.modelId })
  }
  ...
  useEffect(() => {
    function onDown(e: MouseEvent): void {
      ...
      if (modelToolRef.current && !modelToolRef.current.contains(e.target as Node))
  ...
  async function submit(): Promise<void> {
    ...
    if (!text && attachments.length === 0 && active.length === 0 && !hasAt) return

    let display = text
    let agent = text
    if (active.length > 0) {
      const refs = active
        .map((m) => (m.kind === 'symbol' && m.line ? `@${m.path}:${m.line}` : `@${m.path}`))
        .join(' ')
      agent = text ? `${text}\n\n${refs}` : refs
    }
    ...
    if (usedAt.length > 0) {
      ...
      for (const id of usedAt) {
        if (id === 'see') {
          const urlMatch = /\bhttps?:\/\/[^\s]+/i.exec(text)?.[0]
    ...
    onSend(display, attachments.length ? attachments : undefined, agent !== display ? agent : undefined)
    setValue('')
    setAttachments([])
    setMentions([])
    setAtSources([])
    ...
    <div className="ns">
      ...
      <div
        ...
        onDrop={(e) => {
          ...
          if (raw) {
            e.preventDefault()
            try {
              const f = JSON.parse(raw) as { path: string; name: string; isDir?: boolean }
      ...
      >
          ...
          <div className="ns__attachments">
            {attachments.map((a) => (
              <div key={a.id} className="ns__attachment">
                <img src={a.dataUrl} alt={a.name} className="ns__attachment-img" />
                <button
                  type="button"
            ...
            {mentionHits.map((h, i) => (
              ...
              >
                {h.kind === 'dir' ? (
                  ...
                  <img src={fileIcon(h.name)} alt="" className="ns__mention-icon" />
                )}
        ...
        <div className="ns__input-wrap">
          ...
          <textarea
            ...
            onKeyDown={onKeyDown}
            onScroll={(e) => {
              const hl = e.currentTarget.previousElementSibling as HTMLElement | null
        ...
        <div className="ns__row">
          ...
          <div className="ns__chip-wrap" ref={modelToolRef}>
            ...
            {modelToolOpen && models.length > 0 && (
              <div className="ns__menu ns__menu--up" role="menu">
                {models.map((m) => (
                  <button
          ...
          <div className="ns__chip-wrap" ref={accessRef}>
            ...
            >
              {access === 'high' ? <Zap size={13} /> : <ShieldCheck size={13} />}
              <span>{t(access === 'high' ? 'access.high' : 'access.normal')}</span>
              <ChevronDown
                size={12}
            ...
            {accessOpen && (
              <div className="ns__menu ns__menu--up ns__access-menu" role="menu">
                {ACCESS_LEVELS.map((a) => {
                  const Icon = a.icon
                  return (
                    <button
      ...
      <div className="ns__bottom-row">
        <div className="ns__chip-wrap ns__edit-wrap" ref={editRef}>
          <button className="ns__edit" type="button" onClick={() => setEditOpen((v) => !v)}>
            <Pencil size={13} />
            <span>{t(EDIT_MODES.find((m) => m.id === editMode)?.key ?? 'chat.edit.auto')}</span>
            <ChevronDown size={13} className={`ns__chevron${editOpen ? ' ns__chevron--open' : ''}`} />
          </button>
          {editOpen && (
            <div className={`ns__menu${showHeader ? '' : ' ns__menu--up'}`} role="menu">
              {EDIT_MODES.map((m) => (
                <button
                  key={m.id}
                  type="button"
                  ...
                  className="ns__menu-item"
                  onClick={() => {
                    setEditModeState(m.id as EditMode)
                    setEditMode(m.id as EditMode)
                    setEditOpen(false)
                  }}
                >
                  <span className="ns__menu-label">{t(m.key)}</span>
                  {m.id === editMode && <Check size={13} />}
                </button>
              ))}
            </div>
          )}
        ...
        <div className="ns__chip-wrap" ref={sessionRef}>
          ...
          {sessionOpen && (
            <div className="ns__menu ns__menu--up ns__menu--right ns__session-menu" role="menu">
              <button
                type="button"
              ...
              {sessionList.length === 0 ? (
                <div className="ns__session-empty">{t('sessions.emptyNoSessions')}</div>
              ) : (
                sessionList.map((conv) =>
                  sessionRenameId === conv.id ? (
                    ...
                    <button
                      ...
                      onContextMenu={(e) => {
                        e.preventDefault()
                        setSessionMenu({ x: e.clientX, y: e.clientY, id: conv.id })
                      }}
                    >
                      <span className="ns__menu-label">{conv.title}</span>
                      {conv.id === state.activeConversationId && <Check size={13} />}
                    </button>
                  )
                )
              )}
