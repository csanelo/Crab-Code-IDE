...
const api = {
  window: {
    ...
    zoom: (delta: number) =>
      ipcRenderer.invoke("window:zoom", delta) as Promise<number>,
  ...
  app: {
    ...
    copy: (text: string) =>
      ipcRenderer.invoke("app:clipboard-write", text) as Promise<boolean>,
    paste: () => ipcRenderer.invoke("app:clipboard-read") as Promise<string>,
    openAgentWindow: () =>
      ipcRenderer.invoke("app:open-agent-window") as Promise<boolean>,
    returnToIde: (sessionSnapshot?: unknown) =>
      ipcRenderer.invoke(
        "app:return-to-ide",
        sessionSnapshot,
      ) as Promise<boolean>,
    onAgentSessions: (cb: (payload: unknown) => void) => {
      const listener = (_e: unknown, payload: unknown): void => cb(payload);
      ipcRenderer.on("app:agent-sessions", listener);
      return () => ipcRenderer.removeListener("app:agent-sessions", listener);
    },
  ...
  project: {
    openDialog: () =>
      ipcRenderer.invoke("project:open-dialog") as Promise<{
        path: string;
        name: string;
      } | null>,
    reveal: (path: string) =>
      ipcRenderer.invoke("project:reveal", path) as Promise<boolean>,
  },
  fs: {
    openFolder: () =>
      ipcRenderer.invoke("fs:open-folder") as Promise<{
        path: string;
        name: string;
      } | null>,
    openFile: () =>
    ...
    save: (payload: {
      ...
      ipcRenderer.invoke("fs:save", payload) as Promise<{
        path: string;
        name: string;
      } | null>,
    ...
    saveAs: (payload: { content: string; suggestedName?: string }) =>
      ipcRenderer.invoke("fs:save-as", payload) as Promise<{
        path: string;
        name: string;
      } | null>,
    ...
    deletePath: (path: string) =>
      ipcRenderer.invoke("fs:delete", path) as Promise<boolean>,
    ...
    searchSymbols: (root: string, query: string, limit?: number) =>
      ipcRenderer.invoke("fs:search-symbols", {
        root,
        query,
        limit,
      }) as Promise<
  ...
  lsp: {
    ...
    onDiagnostics: (
      cb: (payload: { uri: string; diagnostics: unknown[] }) => void,
    ) => {
      const listener = (
        _e: unknown,
  ...
  workspace: {
    // Fired in every window whenever the agent mutates something on disk,
    // no matter which window started the request.
    onChanged: (
      cb: (payload: {
        tool: string;
        cwd: string | null;
      ...
      const listener = (
        _e: unknown,
        payload: { tool: string; cwd: string | null; path: string | null },
      ): void => cb(payload);
  ...
  editor: {
    open: (path: string) =>
      ipcRenderer.invoke("editor:open", path) as Promise<boolean>,
    ...
    toggleMaximize: () =>
      ipcRenderer.invoke("editor-window:toggle-maximize") as Promise<boolean>,
  ...
  terminal: {
    ...
    spawnRemote: (
      ...
      ipcRenderer.invoke("terminal:spawn-remote", {
        id,
        connId,
        cwd,
        cols,
    ...
    interrupt: (id: string) =>
      ipcRenderer.invoke("terminal:interrupt", id) as Promise<boolean>,
  ...
  agent: {
    ...
    confirmResponse: (confirmId: string, allowed: boolean) =>
      ipcRenderer.send("agent:confirm-response", confirmId, allowed),
    onChunk: (cb: (requestId: string, chunk: string) => void) => {
      const listener = (_e: unknown, id: string, chunk: string): void =>
        cb(id, chunk);
    ...
    onTool: (
      cb: (
        ...
        event: {
          ...
          status: "running" | "done" | "confirm-pending" | "confirm-resolved";
          result?: string;
          meta?: {
            path: string;
            added: number;
    ...
    onError: (cb: (requestId: string, message: string) => void) => {
      const listener = (_e: unknown, id: string, message: string): void =>
        cb(id, message);
  ...
  skills: {
    ...
    addFromRepo: (root: string, url: string, skill: string) =>
      ipcRenderer.invoke("skills:addFromRepo", {
        root,
        url,
        skill,
      }) as Promise<{
    ...
    sync: (root: string) =>
      ipcRenderer.invoke("skills:sync", root) as Promise<boolean>,
  },
  github: {
    getAuth: () =>
    ...
    disconnect: () =>
      ipcRenderer.invoke("github:disconnect") as Promise<{ ok: boolean }>,
    listRepos: () =>
    ...
    pull: (path: string) =>
      ipcRenderer.invoke("github:pull", path) as Promise<{
        ok: boolean;
        error?: string;
      }>,
    commitPush: (payload: {
  ...
  remote: {
    ...
    remove: (id: string) =>
      ipcRenderer.invoke("remote:remove", id) as Promise<boolean>,
    connect: (id: string) =>
      ipcRenderer.invoke("remote:connect", id) as Promise<
        { id: string; label: string; rootPath: string } | { error: string }
      >,
    disconnect: (id: string) =>
      ipcRenderer.invoke("remote:disconnect", id) as Promise<boolean>,
    readDir: (path: string) =>
  ...
  mcp: {
    ...
    remove: (id: string) =>
      ipcRenderer.invoke("mcp:remove", id) as Promise<unknown>,
