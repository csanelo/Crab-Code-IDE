...
export function TerminalPanel({ onClose }: { onClose: () => void }): JSX.Element {
  ...
  function addTab(): void {
    ...
    setTabs((prev) => [...prev, { id, title: `${t('terminal.title')} ${prev.length + 1}` }])
    setActiveId(id)
  }
        ...
        <div className="terminal__tabs">
          {tabs.map((tab) => (
            <div
              ...
              tabIndex={0}
            >
              <span className="terminal__tab-icon terminal__tab-prompt" aria-hidden="true">
                {'>_'}
              </span>
  ...
  useEffect(() => {
    ...
    function trySpawn(): void {
      ...
      const spawnPromise = remote
        ? window.api.terminal.spawnRemote(id, remote.connId, remote.cwd, cols, rows)
