  ...
  switch (action.type) {
    ...
    case "ADD_MESSAGE": {
      ...
      const isFirstUserMessage =
        action.role === "user" && conv.messages.length === 0;
        ...
        messages: conv.messages.map((m) => {
          ...
          if (lastSeg && lastSeg.kind === "text") {
            segments[segments.length - 1] = {
              kind: "text",
              text: lastSeg.text + action.chunk,
            };
          } else {
    ...
    case "TOOL_EVENT": {
        ...
        messages: conv.messages.map((m) => {
          ...
          if (action.tool.status === "done") {
            const idx = [...calls]
              .reverse()
              .findIndex(
                (c) => c.name === action.tool.name && c.status === "running",
              );
            if (idx >= 0) {
      ...
      return {
        ...
        conversations: {
          ...state.conversations,
          [action.conversationId]: updatedConv,
        },
        changes,
      };
    }
    ...
    case "REMOVE_CHANGE": {
      ...
      return {
        ...state,
        changes: { ...state.changes, [action.repositoryId]: next },
      };
    }
      ...
      const updated: Conversation = {
        ...
        messages: conv.messages.map((m) =>
          m.id === action.messageId
            ? { ...m, streaming: false, error: action.error }
            : m,
        ),
      };
      return {
    ...
    case "SYNC_AGENT_SESSIONS": {
      const conversations = { ...state.conversations };
      const repositories = state.repositories.map((repository) => {
        const nextIds = action.payload.repositoryConversationIds[repository.id];
        if (!nextIds) return repository;
        for (const previousId of repository.conversationIds) {
          if (!nextIds.includes(previousId)) delete conversations[previousId];
        }
        for (const id of nextIds) {
          const conversation = action.payload.conversations[id];
          if (conversation) conversations[id] = conversation;
        }
        return { ...repository, conversationIds: [...nextIds] };
      });
      const activeConversationId =
