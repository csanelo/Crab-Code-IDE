...
function shellCommand(): { file: string; args: string[] } {
  ...
  if (pref !== 'auto') {
    if (pref === 'cmd' && isWin) return { file: process.env.ComSpec ?? 'cmd.exe', args: [] }
    if (pref === 'powershell' && isWin) return { file: 'powershell.exe', args: [] }
    if (pref === 'pwsh') return { file: 'pwsh', args: [] }
    if (pref === 'gitbash' && isWin) {
      return { file: 'C:/Program Files/Git/bin/bash.exe', args: ['--login', '-i'] }
    }
...
export function registerTerminal(win: BrowserWindow): void {
  ipcMain.handle(
    'terminal:spawn',
    (_e, payload: { id: string; cwd: string | null; cols: number; rows: number }) => {
      ...
      try {
        proc = pty.spawn(file, args, {
          name: 'xterm-256color',
          cwd: payload.cwd ?? process.env.USERPROFILE ?? process.env.HOME ?? process.cwd(),
      ...
      proc.onData((data) => {
        win.webContents.send('terminal:data', payload.id, data)
      })
      proc.onExit(({ exitCode }) => {
        win.webContents.send('terminal:exit', payload.id, exitCode)
  ...
  ipcMain.handle(
    ...
    async (
      _e,
      payload: { id: string; connId: string; cwd: string | null; cols: number; rows: number }
    ) => {
      const existing = sessions.get(payload.id)
      ...
      return await new Promise<{ pid: number; reused: boolean }>((resolve) => {
        conn.client.shell(
          { cols: Math.max(20, payload.cols | 0) || 80, rows: Math.max(5, payload.rows | 0) || 24 },
          (err, stream) => {
            if (err) {
              win.webContents.send(
                'terminal:data',
            ...
            stream.on('data', (d: Buffer) =>
              win.webContents.send('terminal:data', payload.id, d.toString())
            )
            stream.stderr.on('data', (d: Buffer) =>
              win.webContents.send('terminal:data', payload.id, d.toString())
            )
            stream.on('close', () => {
              win.webContents.send('terminal:exit', payload.id, 0)
              sessions.delete(payload.id)
            })
            sessions.set(payload.id, { remote: stream })
            if (payload.cwd) stream.write(`cd ${JSON.stringify(payload.cwd)}\n`)
  ...
  ipcMain.handle('terminal:resize', (_e, id: string, cols: number, rows: number) => {
    ...
    if (!s) return false
    try {
      if (s.remote) s.remote.setWindow(Math.max(5, rows | 0), Math.max(20, cols | 0), 0, 0)
